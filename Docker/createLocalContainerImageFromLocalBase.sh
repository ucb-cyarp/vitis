...
cd $scriptSrc
echo "docker build -t laminar-remote-cpp-env-local:${timestamp} -f Dockerfile.fromLocal ."
docker build --progress=plain -t "laminar-remote-cpp-env-local:${timestamp}" -f Dockerfile.fromLocal . 2>&1 | tee dockerBuild.log
