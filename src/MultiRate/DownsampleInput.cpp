...
void DownsampleInput::specializeForBlocking(int localBlockingLength, int localSubBlockingLength,
    ...
    if(useVectorSamplingMode){
        ...
        for(const std::shared_ptr<Arc> &arc : inputArcs){
            ...
            int blockingLengthOutsideClkDomain = localBlockingLength*getDownsampleRatio();
            std::get<1>(expansion) = blockingLengthOutsideClkDomain;
