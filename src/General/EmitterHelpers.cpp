...
std::string EmitterHelpers::emitPAPIHelper(std::string path, std::string fileNamePrefix){
    ...
    cFile << "#include <stdio.h>\n"
             ...
             "        printf(\"Error Initializing PAPI Library, possibly due to version mismatch: %d\\n\", status);\n"
             "        exit(1);\n"
             "    }\n"
             "\n"
             "    //Need to enable threading support in PAPI\n"
             "    //See https://bitbucket.org/icl/papi/wiki/PAPI-Parallel-Programs\n"
             "    status = PAPI_thread_init(pthread_self);\n"
             "    if(status != PAPI_OK){\n"
             "        printf(\"Error Initializing PAPI Library threading support: %d\", status);\n"
             "        exit(1);\n"
             "    }\n"
             "\n"
             "    if(PAPI_thread_id() == (unsigned long) -1){\n"
             "        printf(\"Error Initializing PAPI Library threading support: %d\", status);\n"
             "        exit(1);\n"
             "    }\n"
             "\n"
             ...
             "\n"
             "int setupPapiThread(){\n"
             "    //Not calling PAPI_register_thread since the documentation of the function\n"
             "    //specifies that new threads are implicitally detected by PAPI when they\n"
             "    //create eventsets.\n"
             "\n"
             ...
             "    }\n"
             "\n"
             "    //Check that Multiplexing is not enabled for this eventset\n"
             "    if(PAPI_get_multiplex(eventSet)){\n"
             "        printf(\"Error eventset was set with multiplexing enabled\\n\");\n"
             "        exit(1);\n"
             "    }\n"
             "\n"
             "    return eventSet;\n"
