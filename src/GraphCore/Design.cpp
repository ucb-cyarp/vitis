...
Design::emitSingleThreadedCBenchmarkingDriverConst(std::string path, std::string fileName, std::string designName) {
    ...
    std::string makefileContent =  "BUILD_DIR=build\n"
                                   ...
                                   "\n"
                                   "benchmark_" + fileName + "_const: $(OBJS) $(KERNEL_OBJS) $(KERNEL_NO_OPT_OBJS) $(DEPENDS_LIB)\n"
                                   "\t$(CXX) $(INC) $(LIB_DIRS) -o benchmark_" + fileName + "_const $(OBJS) $(KERNEL_OBJS) $(KERNEL_NO_OPT_OBJS) $(LIB)\n"
                                   "\n"
...
void Design::emitSingleThreadedCBenchmarkingDriverMem(std::string path, std::string fileName, std::string designName) {
    ...
    std::string makefileContent =  "BUILD_DIR=build\n"
                                   ...
                                   "\n"
                                   "benchmark_" + fileName + "_mem: $(OBJS) $(KERNEL_OBJS) $(KERNEL_NO_OPT_OBJS) $(DEPENDS_LIB)\n"
                                   "\t$(CXX) $(INC) $(LIB_DIRS) -o benchmark_" + fileName + "_mem $(OBJS) $(KERNEL_OBJS) $(KERNEL_NO_OPT_OBJS) $(LIB)\n"
                                   "\n"
                                   "$(KERNEL_NO_OPT_OBJS): $(BUILD_DIR)/%.o : $(SRC_DIR)/%.cpp | $(BUILD_DIR)/ $(DEPENDS)\n"
                                   "\t$(CXX) $(KERNEL_NO_OPT_CFLAGS) $(INC) $(DEFINES) -o $@ $<\n"
                                   "\n"
