 ...
 * \f$2\sqrt{\frac{3}{2\left(M-1\right)}}\f$.
 *
 ...
 */
class DigitalDemodulator : public MediumLevelNode{
friend NodeFactory;
...
private:
    ...
    //==== Constructors ====
    /**
     * @brief Constructs a Digital Demodulator node
     *
    ...
    /**
     * @brief Constructs a Digital Demodulator node with a given parent.  This node is not added to the children list of the parent.
     *
     * @note To construct from outside of hierarchy, use factories in @ref NodeFactory
     *
     * @param parent parent node
     */
    explicit DigitalDemodulator(std::shared_ptr<SubSystem> parent);
     ...
     */
    DigitalDemodulator(std::shared_ptr<SubSystem> parent, DigitalDemodulator* orig);
...
public:
     ...
     */
    static std::shared_ptr<DigitalDemodulator> createFromGraphML(int id, std::string name,
                                                               std::map<std::string, std::string> dataKeyValueMap,
                                                               std::shared_ptr<SubSystem> parent, GraphMLDialect dialect);

    //==== Expand ====
    /**
     * @brief Expands the Demodulator
     *
